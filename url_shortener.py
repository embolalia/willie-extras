...
def title_auto(bot, trigger):
    ...
    for url in urls:
	params = urllib.urlencode({'url': url, 'c': "redir_add", 'bml': 1})
	short_url_page = urllib2.urlopen("http://www.dy.fi/", params).read()
	short_url = lxml.html.fromstring(short_url_page).xpath('//a[starts-with(@href, "http://dy.fi/")]/text()')
	for str in short_url:
		bot.say("Shortened: " + str.decode("utf-8"))
